...
TEMPLATE_TAUTO = {
    "T1: (((p → q) → p) → p)":
        dict(vars=["p","q"],
             parts=["Implies(Implies(Implies(p,q),p),p)"]),
    "T2: (p→(q∧r)) → ((p→q) ∧ (p→r))":
        dict(vars=["p","q","r"],
...
TEMPLATE_INFER = {
    "I1: Classi di Peter":
        dict(vars=["PE","PS","PJ"],
             ...
             conclusion="PE"),
    "I2: Mary/Lucia/Pioggia":
        dict(vars=["MG","LG","R"],
             premises=["Implies(LG,MG)","Implies(R,Not(LG))"],
             conclusion="Implies(Not(R),MG)"),
    "I3: Biglietto/Prenotazione":
        dict(vars=["FT","MR"],
             premises=["Implies(FT,MR)","Not(MR)"],
             conclusion="Not(FT)"),
    "I4: Gruppo (Paul/Mary/John/Peter/Andrew)":
        dict(vars=["PaJ","MJ","JJ","PeJ","AJ"],
...
TEMPLATE_COLORING = {
    "America Centrale (K=3)":
        dict(K=3,
             nodes="Bel Gua ElS Hon Nic CR Pan",
             edges="Bel,Gua\nElS,Hon\nElS,Gua\nGua,Hon\nNic,Hon\nNic,CR\nNic,Pan"),
    "Sud America (UNSAT con K=3)":
        dict(K=3,
             nodes=("Ven Col Ecu Per Guy Sur FGu Bra Bol Par Uru Arg Chi"),
             edges=("\n".join([
               "Guy,Ven","Guy,Bra","Guy,Sur","Ven,Col","Ven,Bra","Bra,Sur",
               "FGu,Sur","FGu,Bra","Col,Ecu","Col,Per","Col,Bra","Per,Ecu",
               "Per,Bra","Per,Bol","Per,Chi","Bra,Bol","Par,Bol","Par,Arg",
               "Arg,Bol","Chi,Bol","Bra,Arg","Bra,Par","Bra,Uru",
               "Arg,Chi","Arg,Uru"
             ]))),
}
