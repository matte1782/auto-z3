...
mode = st.sidebar.selectbox(
    "Scegli la modalità",
    [
        "Builder formule (STRICT)",
        "Preset comuni (con esempi del prof)",
        "Coloring K",
        "Tester veloce",
    ],
...
if mode.startswith("Builder"):
    ...
    help_info(
        "Costruisci formule complesse selezionando sotto-formule e inglobandole in un operatore. Sintassi SMT-LIB v2 garantita."
    )

    vars_line = st.text_input(
        "Variabili booleane (spazio-separate)",
        value="p q r s",
    ...
    if "pool" not in st.session_state:
        st.session_state.pool: List[Expr] = vars_.copy()
    if "phi_idx" not in st.session_state:
        st.session_state.phi_idx = None
            ...
            if not select_ids:
                ...
                try:
                    if op == "Not" and len(items) == 1:
                        newf = Not(items[0])
                    elif op == "And" and len(items) >= 2:
                        newf = And(*items)
                    elif op == "Or" and len(items) >= 2:
                        newf = Or(*items)
                    elif op == "Implies" and len(items) == 2:
                        newf = Implies(items[0], items[1])
                    elif op == "Iff" and len(items) == 2:
                        newf = Iff(items[0], items[1])
                    elif op == "Xor" and len(items) == 2:
                        newf = Xor(items[0], items[1])
                    else:
                        st.warning("Cardinalità argomenti non corretta per l’operatore scelto.")
                        raise ValueError
    ...
    with colR:
        ...
        task = st.selectbox(
            "Seleziona task",
            [
                "Tautologia (assert (not Φ))",
                "SAT (assert Φ)",
                "Equivalenza (Φ = Ψ)",
                "Inferenza (Premesse ⊢ Φ)",
            ],
        ...
        if task == "Equivalenza (Φ = Ψ)":
            ...
            try:
                ...
                psi_smt = emit_expr(psi)
                extras = psi_smt
                st.caption("Anteprima Ψ (SMT-LIB):")
                st.code(psi_smt, language="lisp")
            except Exception:
                st.warning("Ψ non valida.")
                psi = None
        elif task == "Inferenza (Premesse ⊢ Φ)":
            prem_txt = st.text_area(
                "Premesse (DSL, una per riga)",
                height=120,
        ...
        if st.button("▶️ Genera & Verifica con Z3"):
            ...
            if st.session_state.phi_idx is None:
                st.warning("Imposta Φ prima di procedere.")
                st.stop()
            phi = st.session_state.pool[st.session_state.phi_idx]
            phi_smt = emit_expr(phi)
            body = []
            if task.startswith("Tautologia"):
                body += [assert_line(f"(not {phi_smt})")]
            elif task.startswith("SAT"):
                body += [assert_line(phi_smt)]
            elif task.startswith("Equivalenza"):
                if not extras:
                    st.warning("Definisci Ψ valida.")
                    st.stop()
                ...
                for ln in lines:
                    body.append(assert_line(emit_expr(eval(ln, {"__builtins__": {}}, env))))
...
# ============ 2) PRESET ============
    ...
    with tabs[0]:
        ...
        with colA:
            preset = st.selectbox(
                "Carica esempio del professore", ["—"] + list(TEMPLATE_TAUTO.keys())
            )
        with colB:
            default_vars = "p q r"
            if preset != "—":
        ...
        if st.button("▶️ Verifica Tautologia"):
            ...
            parts = [
                eval(ln.strip(), {"__builtins__": {}}, env)
                for ln in lines.splitlines()
                if ln.strip()
            ]
            phi = parts[0] if len(parts) == 1 else And(*parts)
            smt = "\n".join(
                [declare_block(vs), assert_line(f"(not {emit_expr(phi)})"), check_sat()]
            )
    ...
    with tabs[1]:
        ...
        with colA:
            preset = st.selectbox(
                "Carica esempio del professore", ["—"] + list(TEMPLATE_INFER.keys())
            )
        if preset != "—":
        ...
        if st.button("▶️ Verifica Inferenza"):
            ...
            body = [
                assert_line(emit_expr(eval(ln.strip(), {"__builtins__": {}}, env)))
                for ln in prem_txt.splitlines()
                if ln.strip()
            ]
    ...
    with tabs[2]:
        ...
        with colA:
            preset = st.selectbox(
                "Carica esempio del professore", ["—"] + list(TEMPLATE_SAT.keys())
            )
        if preset != "—":
    ...
    with tabs[3]:
        ...
        with colA:
            preset = st.selectbox(
                "Carica esempio del professore", ["—"] + list(TEMPLATE_EQUIV.keys())
            )
        if preset != "—":
        ...
        f2 = st.text_input(
            "f2 (DSL)",
            value=st.session_state.get("eq_f2", "And(Implies(p,r), Implies(q,r))"),
        )
...
# ============ 3) COLORING K ============
    ...
    help_info(
        "Per ogni nodo X generiamo X1..XK ∈ Bool con ExactlyOne; per ogni arco (u,v) vietiamo lo stesso colore su entrambi. (Cardinalità via PB `((_ at-most 1) ...)`)."
    )

    colA, colB = st.columns([1, 1])
    with colA:
        preset = st.selectbox(
            "Carica esempio del professore", ["—"] + list(TEMPLATE_COLORING.keys())
        )
        if preset != "—":
    ...
    K = st.number_input(
        "K (colori)", min_value=2, max_value=10, value=st.session_state.get("col_K", 3)
    )
    nodes_line = st.text_input(
        "Nodi (spazio-separati)",
        value=st.session_state.get("col_nodes", "Bel Gua ElS Hon Nic CR Pan"),
    )
    edges_text = st.text_area(
        "Archi u,v (uno per riga)",
        height=140,
    ...
    if st.button("▶️ Genera SMT-LIB + Verifica"):
        ...
        for ln in edges_text.splitlines():
            ...
            edges.append((u, v))
        if bad_edges:
            st.error("Archi mal formati o riferiti a nodi inesistenti: " + ", ".join(bad_edges))
            st.stop()
...
# ============ 4) TESTER ============
    ...
    help_info(
        "Incolla SMT-LIB già pronto (o generato qui) e verifica. Il modello viene richiesto *solo se* `sat`."
    )
    smt = st.text_area(
        "SMT-LIB",
        height=220,
        placeholder="(declare-const p Bool)\n(assert p)\n(check-sat)",
    )
