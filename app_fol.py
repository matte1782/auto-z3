...
from fol_core import (
    VarT, Const, Func, Pred, Eq, Not, And, Or, Implies, Iff, ForAll, Exists,
    make_env, build_smt2_universe, emit_fml
)
...
st.caption("Guidato, semplice: dominio finito, simboli, fatti, quantificatori. "
           "Per risultati deterministici usa **Espansione su dominio finito** (consigliata).")

# --- Stato
if "preds" not in st.session_state: st.session_state.preds: Dict[str,int] = {}
if "funs" not in st.session_state:  st.session_state.funs:  Dict[str,int] = {}
...
with colL:
    ...
    with st.expander("Predicati (nome / arità)", expanded=True):
        ...
        for nm, ar in list(st.session_state.preds.items()):
            st.text_input(f"{nm}/{ar}", value=f"{nm} : {ar}", key=f"pred_{nm}", disabled=True, label_visibility="collapsed")
    ...
    with st.expander("Funzioni (nome / arità)", expanded=False):
        ...
        for nm, ar in list(st.session_state.funs.items()):
            st.text_input(f"{nm}/{ar}", value=f"{nm} : {ar}", key=f"fun_{nm}", disabled=True, label_visibility="collapsed")
            if st.button(f"🗑️ Rimuovi {nm}", key=f"delf_{nm}"):
                st.session_state.funs.pop(nm, None)

    st.subheader("2) Fatti (ground) — opzionali")
    facts_text = st.text_area("Un fatto per riga (es.: P(a,b), Eq(f(a),b), a=b)", height=110,
                              placeholder="Human(a)\nLoves(a,b)\nEq(f(a), a)")
    facts = [ln.strip() for ln in facts_text.splitlines() if ln.strip()]

with colR:
    st.subheader("3) Formula con quantificatori")
    qmode = st.selectbox("Selettore", ["SAT (assiomi + formula)", "Validità (tautologia)", "Conseguenza logica (⊢)"])
    use_ground = st.checkbox("Espansione su dominio finito (consigliata)", value=True,
    ...
    st.caption("Scrivi variabili (spazio-separate), poi la formula interna in DSL. "
    ...
    qtype = st.selectbox("Tipo quantificatore", ["∀ ForAll", "∃ Exists", "Nessuno (formula ground)"])
    inner_str = st.text_input("Formula interna (DSL)", value="Implies(P(x,y), P(y,x))",
                              help="Esempi: P(x), And(P(x), Q(y)), Implies(Eq(f(x),y), R(y))")

    # goal per conseguenza
    goal_str = ""
    if qmode.startswith("Conseguenza"):
        goal_str = st.text_input("Conclusione (DSL, stessa sintassi)", value="Exists([x], P(x,x))".replace("[","").replace("]",""),
    ...
    if st.button("▶️ Genera SMT-LIB + Verifica"):
        try:
            ...
            smt_task = "sat" if qmode.startswith("SAT") else ("validity" if qmode.startswith("Validità") else "inference")
            ...
            badge = {"sat":"✅ SAT","unsat":"🧱 UNSAT","unknown":"❔ UNKNOWN","error":"⚠️ ERROR"}.get(status,status)
...
st.caption("Se hai scritto fatti ground come `Edge(a,b)` o `Loves(a,b)`, seleziona il predicato per vedere il grafo.")
...
# aggiorna cache raw
st.session_state["facts_raw_cache"] = facts_text.splitlines() if 'facts_text' else facts_text.splitlines()

if pname:
    pairs = []
    pat = re.compile(rf"^\s*{re.escape(pname)}\s*\(\s*([A-Za-z0-9_]+)\s*,\s*([A-Za-z0-9_]+)\s*\)\s*$")
    for ln in facts:
        m = pat.match(ln)
        if m:
